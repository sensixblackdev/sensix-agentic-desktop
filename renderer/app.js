(function () {
  ...
  function renderModelOptions() {
    ...
    els.modelSelect.innerHTML = autoOption + (models.length
      ...
      : '');
    if (currentSession) currentSession.model = selected;
  }
    ...
    if (content.startsWith('/')) {
      ...
      if (cmd === '/learning') {
        try {
          ...
          if (stats && stats.lessons && stats.lessons.length > 0) {
            const list = stats.lessons.map((l) => `• **${l.tool}**: ${l.symptom} ➔ *Solução*: \`${l.fixApplied}\``).join('\n');
            session.messages.push({
              id: `msg-${Date.now()}`,
              role: 'assistant',
              content: `🧠 **Auto-Learning Ledger (${stats.totalLessons} lições em memória):**\n\n${list}\n\n*Total de injeções de prevenção*: ${stats.totalInjections}`
            });
          } else {
            session.messages.push({
              id: `msg-${Date.now()}`,
              role: 'assistant',
              content: `🧠 **Auto-Learning Ledger**: Nenhuma lição gravada ainda. O sistema aprende automaticamente com falhas auto-curadas de ferramentas.`
            });
          }
        } catch (e) {
          session.messages.push({ id: `msg-${Date.now()}`, role: 'assistant', content: `Erro ao buscar lições: ${e.message}` });
        }
        persistSessions();
        renderMessages();
        return;
